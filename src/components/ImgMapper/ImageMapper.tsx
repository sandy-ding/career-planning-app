...
import { getExtendedArea, getMatchExtendedArea } from "./helpers/area";
...
const ImageMapper = forwardRef<RefProperties, Required<ImageMapperProps>>(
  (props, ref) => {
    const {
      src,
      map,
    ...
    const renderPrefilledAreas = useCallback(() => {
      map.areas.forEach((area) => {
        const extendedArea = getExtendedArea(
          area,
          { img, ...scaleCoordsParams },
          areaParams
        );
    ...
    const onHighlightArea = (area: MapArea): void => {
      ...
      const { id } = area;
      const ids = [...chosenIds, id];
      setChosenIds(ids);

      const chosenArea = isMulti
        ? area
        : areasRef.find((c) => c[areaKeyName] === area[areaKeyName]);
      ...
      drawShape(extendedArea, ctx);

      if (area.matchId) {
        const matchedArea = getMatchExtendedArea(
          chosenArea,
          map,
          { img, ...scaleCoordsParams },
          areaParams
        ...
        if (matchedArea) {
          drawShape(matchedArea, ctx);
        }
      }

      onChange(newArea, areas, ids);
    };
    ...
    const renderAreas = (): ReactNode =>
      map.areas.map((area, index) => {
        const { scaledCoords } = getExtendedArea(
          area,
          { img, ...scaleCoordsParams },
          areaParams
        );
...
const ImageMapperRequired = forwardRef<RefProperties, ImageMapperProps>(
  (props, ref) => <ImageMapper ref={ref} {...generateProps(props)} />
);

ImageMapperRequired.displayName = "ImageMapperRequiredForwarded";

export default memo(ImageMapperRequired, (prevProps, nextProps) => {
  const propChanged = rerenderPropsList.some(
