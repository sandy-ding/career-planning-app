...
type RequiredArea<
  T extends MapArea = MapArea,
  R extends keyof T = RequiredMapArea
> = Omit<T, R> & Pick<NoUndefinedField<T>, R>;
...
export type EventHandler<T = AreaEvent> =
  | ((area: MapArea, index: number, e: T) => void)
  | null;
export type LoadEventHandler =
  | ((event: HTMLImageElement, dimensions: WidthHeight) => void)
  | null;
