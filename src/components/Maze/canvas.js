...
const MOVEMENT_KEY_NAMES = ["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"];
/** Keys that make the page scroll when pressed. */
const SCROLLING_KEY_NAMES = [
  " ",
  "ArrowUp",
  "ArrowDown",
  "ArrowLeft",
...
function getValuesFromOptions(image, options) {
  ...
  const wallLuminosityThreshold =
    options.wallLuminosityThreshold === undefined
      ? 190
      : options.wallLuminosityThreshold;
  // The default goal position is the bottom right corner.
  const goalX =
    options.goalX === undefined
      ? image.naturalWidth - GOAL_SIZE_DEFAULT
      : options.goalX;
  const goalY =
    options.goalY === undefined
      ? image.naturalHeight - GOAL_SIZE_DEFAULT
      : options.goalY;
  const goalWidth =
    options.goalWidth === undefined ? GOAL_SIZE_DEFAULT : options.goalWidth;
  const goalHeight =
    options.goalHeight === undefined ? GOAL_SIZE_DEFAULT : options.goalHeight;
  return {
    startX,
    startY,
    wallLuminosityThreshold,
    goalX,
    goalY,
    goalWidth,
    goalHeight,
 ...
 * @param {MazeOptions|undefined} options
 */
export const initMaze = function (canvasId, imageId, options) {
  ...
  const {
    ...
    goalWidth,
    goalHeight,
  } = getValuesFromOptions(image, options);
  ...
  const MOVER_MOVE_SPEED = options?.moveSpeed || 1.1; // of Mover
  ...
  Mover.prototype.update = function () {
    ...
    const repulsion = getRepulsionFromDark(this.pos, 7).multiply(10);
    if (repulsion.distanceSquared() > 1 && !hasWon) {
      // console.log({ repulsion });
      canvas.dispatchEvent(
        new CustomEvent("collision", { bubbles: true, composed: true })
      );
    }
    // console.log("keyboardDirection", keyboardDirection);
  ...
  Vector2.prototype.normalise = function () {
    ...
    if (magnitude === 0) {
      // don't change anything if 0 vector
      return this;
    }
  ...
  function getRepulsionFromDark(midPos, diameter) {
    ...
    for (let y = yTopLeft; y < yTopLeft + diameter; y++) {
      for (let x = xTopLeft; x < xTopLeft + diameter; x++) {
        if (
          !inBounds(x, canvas.width) ||
          !inBounds(y, canvas.height) ||
          hasWall[y][x]
        ) {
          // out of bounds or wall pixel
          // vector from pixel pointing to middle position
          const pixelToMid = new Vector2(
            midPosXRounded - x,
            midPosYRounded - y
          );
  ...
  console.log(
    "Started a maze with canvas of  w: " +
      canvas.width +
      ", h: " +
      canvas.height
  );
  ...
  function animate() {
    ...
    if (
      !hasWon &&
      mover.pos.x >= goalX &&
      mover.pos.x <= goalX + goalWidth &&
      mover.pos.y >= goalY &&
      mover.pos.y <= goalY + goalHeight
    ) {
      hasWon = true;
